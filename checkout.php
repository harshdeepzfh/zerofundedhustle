...
// 1. Set your PayU environment
// ------------------------------------
// For LIVE payments, use: https://secure.payu.in
// For TEST payments, use: https://test.payu.in
$PAYU_BASE_URL = "https://secure.payu.in";

// 2. Add your Merchant Credentials
// ------------------------------------
// These are from your PayU Dashboard.
$MERCHANT_KEY = "GnVkSn"; 
$MERCHANT_SALT = "R7qVqGqCjlKqzbSIHDFLovL0TSnmAw4Zy";

// 3. Set URLs for Success and Failure
// ------------------------------------
// These MUST be full, absolute URLs.
// Update "zerofundedhustle.com" if your domain is different.
$SUCCESS_URL = "https://zerofundedhustle.com/success"; // Your 'surl'
$FAILURE_URL = "https://zerofundedhustle.com/failure"; // Your 'furl'

// 4. Set Product Details
// ------------------------------------
// These are the final values you provided.
$amount = "299.00";
$productinfo = "ESCAPE THE 9-5 | ZEROFUNDEDHUSTLE";

// 5. Generate Unique Transaction ID
// ------------------------------------
// This creates a unique ID for every order.
$txnid = substr(hash('sha256', mt_rand() . microtime()), 0, 20);

// --- DO NOT EDIT BELOW THIS LINE ---
// --- HASH CALCULATION ---

// This is the data string that PayU requires for the hash
$hash_string = $MERCHANT_KEY . '|' . $txnid . '|' . $amount . '|' . $productinfo . '|' .
               // Placeholder fields for user data (will be filled by the form)
               '' . '|' . '' . '|' . '' . '|' . '' . '|' . '' . '|' . '' . '|' . '' . '|' . '' . '|' . '' . '|' . '' . '|' . '' . '|' .
               $MERCHANT_SALT;
...
<head>
    ...
    <!-- PayU Auto-submit Script -->
    <script>
        // This script will auto-submit the form as soon as the page loads.
        // The user will only see this page for a split second before being
        // redirected to the PayU payment gateway.
...
<!-- 
  NOTE: We call submitPayuForm() on body load. This is a standard PayU flow.
  The user fills in their details (name, email) on THIS page.
  When they click "Proceed to Pay", the PHP logic at the top of the file
  has already run, calculated the hash, and prepared the hidden fields.
  The form then submits to PayU.
  
  Wait, the user IS supposed to enter details here. 
  Let's change this. The user should *see* the form.
  
  --- CORRECTION ---
  The user must enter their name and email *before* the hash is calculated.
  The PHP logic must run *after* the form is submitted.
  
  Let's re-structure this to a 2-step process, which is more standard.
  
  Step 1: User enters name/email and clicks "Pay".
  Step 2: The form submits *to this same page* (checkout.php).
  Step 3: The PHP logic at the top runs, catches the POST data,
          generates the hash, and *then* auto-submits to PayU.
          
  This is the correct, secure flow.
-->
